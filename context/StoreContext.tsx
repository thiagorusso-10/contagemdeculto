...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      // If not logged in, maybe clear data or keep empty?
      setLoading(false);
    }
  }, [user]);
  ...
  const addReport = async (report: Report) => {
    ...
    // DB Insert
    // Need to handle preacher. If ID exists in preachers list, use it. If it's a new name? 
    // The current form might only send IDs if selected. 
    // Assuming report.preacherId is a UUID from the selector.

    const dbPayload = {
      // id: report.id, // Let DB generate ID or use the one generated by frontend? 
      // Frontend usually generates temp ID. Better to let DB generate and update?
      // For simplicity, let's try to trust the frontend ID if it's a UUID, OR omit it.
      // Usually better to omit ID on insert if auto-gen.
      // BUT, optimistic update uses the ID. 
      // Let's use the ID provided by frontend if it is a valid UUID, otherwise let DB gen.
      // Actually, let's just insert standard fields.
    ...
    if (error) {
      ...
      // Update the local report with the real ID from DB to avoid issues later
  ...
  // Volunteer Areas are strictly local/frontend config for now based on requirements analysis
  const addVolunteerArea = (name: string) => {
    const newArea: VolunteerArea = { id: Date.now().toString(), name };
    setState(prev => ({ ...prev, volunteerAreas: [...prev.volunteerAreas, newArea] }));
  };

  const deleteVolunteerArea = (id: string) => {
